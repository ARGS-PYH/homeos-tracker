...
import { BUSINESS as DEFAULT_BUSINESS, DEV as DEFAULT_DEV } from './data.js'
...
// ── Helpers ───────────────────────────────────────────────────────────────────
const taskKey  = (tab, gi, ii, item) => item ? item.id : `${tab}__${gi}__${ii}`
const pct      = (done, total) => total === 0 ? 0 : Math.round((done / total) * 100)
const allKeys  = (data, tab) => data.flatMap((g, gi) => g.items.map((item, ii) => taskKey(tab, gi, ii, item)))
const phaseKeys = (data, tab, phase) =>
  data.filter(g => g.phase === phase).flatMap(g => {
    const gi = data.indexOf(g)
    return g.items.map((item, ii) => taskKey(tab, gi, ii, item))
  })

const getTaskLabel = (key, tasksConfig) => {
  const [tab, gi, ii] = key.split('__')
  const group = tab === 'business' ? tasksConfig?.business?.[Number(gi)] : tasksConfig?.dev?.[Number(gi)]
  return group?.items?.[Number(ii)]?.text || key
}

const getLatestAction = (checkedData, tasksConfig) => {
  const actions = Object.entries(checkedData)
    .filter(([key, value]) => key.endsWith('__meta') && value && value.by && value.at)
    .map(([key, meta]) => {
      const taskKeyStr = key.replace(/__meta$/, '')
      const timestamp = new Date(meta.at).getTime() || 0
      return {
        taskKey: taskKeyStr,
        taskLabel: getTaskLabel(taskKeyStr, tasksConfig),
        by: meta.by,
        at: meta.at,
        timestamp,
      }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ width: '100%', maxWidth: 360, background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: '2rem', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: '1.25rem' }}>Peregrine Solutions · Internal tool</p>
      </div>
...
function Group({ group, gi, tab, checked, onToggle, userName, editMode, onUpdateGroup, onDeleteGroup, onUpdateTask, onDeleteTask, onAddTask }) {
  const keys   = group.items.map((item, ii) => taskKey(tab, gi, ii, item))
  ...
  return (
    <div style={{
      ...
      <div className="group-header" style={{
        ...
        background: allDone ? G_LITE : '#F9FAFB', borderBottom: '1px solid #E5E7EB'
      }}>
        {editMode ? (
          <select value={group.phase} onChange={e => onUpdateGroup(gi, 'phase', Number(e.target.value))} style={{ padding: 4, borderRadius: 4, border: '1px solid #ddd' }}>
          ...
          <span style={{
            ...
            background: group.phase === 1 ? G : AMBER_BG, color: group.phase === 1 ? '#fff' : AMBER
          }}>Phase {group.phase}</span>
        )}
        
        {editMode ? (
          <input value={group.title} onChange={e => onUpdateGroup(gi, 'title', e.target.value)} style={{ flex: 1, padding: 4, borderRadius: 4, border: '1px solid #ddd' }} />
        ) : (
          <span style={{ fontWeight: 500, fontSize: 14, color: '#1A1A1A', flex: 1 }}>{group.title}</span>
        )}
        
        {editMode ? (
          <button onClick={() => onDeleteGroup(gi)} style={{ color: '#EF4444', background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>❌</button>
        ) : (
          <span style={{ fontSize: 12, color: allDone ? G : '#9CA3AF', fontWeight: allDone ? 600 : 400 }}>{done}/{group.items.length}</span>
        )}
      </div>

      {!editMode && <div className="task-group" style={{ padding: '0 16px' }}><Bar done={done} total={group.items.length} height={3} /></div>}

      {group.items.map((item, ii) => {
        const k    = taskKey(tab, gi, ii, item)
        ...
        return (
          <div key={ii} style={{
            ...
            borderTop: '1px solid #F3F4F6', background: isDone && !editMode ? G_LITE : 'transparent',
            transition: 'background 0.15s'
          }}>
            {!editMode && (
              <input type="checkbox" checked={isDone} onChange={() => onToggle(k, userName)} style={{ marginTop: 2, width: 16, height: 16, cursor: 'pointer' }} />
            )}
            <div style={{ flex: 1 }}>
              {editMode ? (
                <textarea value={item.text} onChange={e => onUpdateTask(gi, ii, e.target.value)} style={{ width: '100%', padding: 4, borderRadius: 4, border: '1px solid #ddd', minHeight: 40 }} />
              ) : (
                <span style={{ fontSize: 13, lineHeight: 1.5, color: isDone ? '#9CA3AF' : '#1A1A1A', textDecoration: isDone ? 'line-through' : 'none', cursor: 'pointer' }} onClick={() => onToggle(k, userName)}>
                  {item.text}
                </span>
              )}
              {isDone && meta && !editMode && (
                <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>✓ {meta.by} · {meta.at}</div>
              )}
            </div>
            {editMode && (
              <button onClick={() => onDeleteTask(gi, ii)} style={{ color: '#EF4444', background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>🗑️</button>
            )}
          </div>
        )
      })}
      ...
      {editMode && (
        <div style={{ padding: '10px 16px', borderTop: '1px solid #F3F4F6' }}>
          <button onClick={() => onAddTask(gi)} style={{ fontSize: 12, color: G, background: G_LITE, border: 'none', padding: '6px 12px', borderRadius: 4, cursor: 'pointer', fontWeight: 500 }}>+ Add Task</button>
        </div>
      )}
    </div>
  )
...
export default function App() {
  ...
  const [tasksConfig, setTasksConfig] = useState({ business: DEFAULT_BUSINESS, dev: DEFAULT_DEV })
  ...
  const toggleClientTask = useCallback(async (key, name, isChecked, time) => {
    if (!isFirebaseConfigured || !db) throw new Error('Firebase client is not configured')
    const ref = doc(db, 'homeos', 'tasks')
    const update = {
      [key]: isChecked,
  ...
  useEffect(() => {
    ...
    const ref = doc(db, 'homeos', 'tasks_config')
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists() && snap.data().business) {
        setTasksConfig(snap.data())
      } else {
        setDoc(ref, { business: DEFAULT_BUSINESS, dev: DEFAULT_DEV })
        setTasksConfig({ business: DEFAULT_BUSINESS, dev: DEFAULT_DEV })
      }
    })
    return unsub
  }, [])
  ...
  useEffect(() => {
    setLastAction(getLatestAction(checked, tasksConfig))
  }, [checked, tasksConfig])
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (!contentType.includes('application/json')) { setConnected(false); return }
        setConnected(true)
  ...
  useEffect(() => {
    ...
    if (!isFirebaseConfigured || !db) {
      const updatePresence = async () => {
        try {
          await fetch(apiUrl('/api/presence/report'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: presenceId, name: userName, tab }) })
      ...
      const loadPresence = async () => {
        ...
        } catch (error) { console.warn('Presence load failed:', error) }
      }
      updatePresence()
      loadPresence()
      const presenceInterval = setInterval(updatePresence, 5000)
      const loadInterval = setInterval(loadPresence, 5000)
      return () => { clearInterval(presenceInterval); clearInterval(loadInterval) }
    }
    ...
    const updatePresence = async () => {
      try {
        await setDoc(presenceRef, { name: userName, tab, lastSeen: serverTimestamp() }, { merge: true })
      } catch (error) { console.warn('Presence update failed:', error) }
    }
    updatePresence()
    const presenceInterval = setInterval(updatePresence, 5000)
    ...
    const unsubscribePresence = onSnapshot(presenceQuery, (snap) => {
      const now = Date.now()
      const users = snap.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() })).filter((user) => {
        if (!user.name) return false
        if (user.lastSeen?.toMillis) return (now - user.lastSeen.toMillis()) < 30000
        return true
      })
      setActiveUsers(users)
    }, (error) => console.warn('Presence listener failed:', error))
    return () => { clearInterval(presenceInterval); unsubscribePresence() }
  }, [authed, nameSet, userName, tab])

  const toggle = useCallback(async (key, name) => {
    const now = new Date()
    const time = now.toLocaleDateString('en-NG', { day: 'numeric', month: 'short' }) + ' ' + now.toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit' })
    const isChecked = !checked[key]
    setChecked(prev => ({ ...prev, [key]: isChecked, [`${key}__meta`]: isChecked ? { by: name || 'Team', at: time } : null }))
    setLastUpdate(time)
    try { localStorage.setItem('homeos_tasks', JSON.stringify({ ...checked, [key]: isChecked, [`${key}__meta`]: isChecked ? { by: name || 'Team', at: time } : null })) } catch {}
    ...
    const apiWrite = shouldUseApiWrite
      ? fetch(apiUrl('/api/tasks/toggle'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key, userName: name, isChecked }) }).then(r => {
          if (!r.ok) throw new Error(`API ${r.status}`)
          if (!(r.headers.get('content-type') || '').includes('application/json')) throw new Error('API returned non-JSON response')
        })
    ...
    if (anyOk) { setConnected(true) } else {
      console.error('All write paths failed — reverting', results.map(r => r.reason))
      setConnected(false)
      setChecked(prev => ({ ...prev, [key]: !isChecked, [`${key}__meta`]: prev[`${key}__meta`] ?? null }))
    }
  }, [checked, toggleClientTask])

  const saveTasksConfig = async (newConfig) => {
    setTasksConfig(newConfig)
    if (isFirebaseConfigured && db) {
      await setDoc(doc(db, 'homeos', 'tasks_config'), newConfig)
    }
  }

  const handleAddGroup = () => {
    const newConfig = { ...tasksConfig }
    newConfig[tab] = [...newConfig[tab], { id: `${tab}_group_${Date.now()}`, phase: 1, title: 'New Phase', items: [] }]
    saveTasksConfig(newConfig)
  }

  const handleUpdateGroup = (gi, field, value) => {
    const newConfig = { ...tasksConfig }
    newConfig[tab][gi][field] = value
    saveTasksConfig(newConfig)
  }

  const handleDeleteGroup = (gi) => {
    if (!window.confirm('Delete this entire phase?')) return
    const newConfig = { ...tasksConfig }
    newConfig[tab].splice(gi, 1)
    saveTasksConfig(newConfig)
  }

  const handleAddTask = (gi) => {
    const newConfig = { ...tasksConfig }
    newConfig[tab][gi].items.push({ id: `${tab}__${gi}__${Date.now()}`, text: 'New Task' })
    saveTasksConfig(newConfig)
  }

  const handleUpdateTask = (gi, ii, text) => {
    const newConfig = { ...tasksConfig }
    newConfig[tab][gi].items[ii].text = text
    saveTasksConfig(newConfig)
  }

  const handleDeleteTask = (gi, ii) => {
    const newConfig = { ...tasksConfig }
    newConfig[tab][gi].items.splice(ii, 1)
    saveTasksConfig(newConfig)
  }
  ...
  if (!authed) return <PinGate onUnlock={(name) => { setAuthed(true); setUserName(name); setNameSet(true) }} />
  if (!nameSet) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
        <div style={{ width: '100%', maxWidth: 360, background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: '2rem', textAlign: 'center' }}>
          ...
          <input
            type="text" placeholder="Your first name" defaultValue={userName} id="nameInput"
            ...
            autoFocus
          />
          <button onClick={() => saveName(document.getElementById('nameInput').value)} style={{ width: '100%', padding: '10px', background: G, color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500 }}>
            Let's go
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F9FAFB' }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(255,255,255,0.95)', borderBottom: '1px solid #E5E7EB', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
        <div className="nav-inner nav-content" style={{ maxWidth: 860, margin: '0 auto', padding: '0 1.25rem', height: 56, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div className="nav-status" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
            ...
            {activeUsers.length > 0 && <div className="nav-status-line" style={{ fontSize: 10, color: '#6B7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 240 }}>Active: {activeUsers.map(u => u.name).join(', ')}</div>}
            {lastAction && <div className="nav-status-line" style={{ fontSize: 10, color: '#6B7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 240 }}>Last action: {lastAction.by} checked "{lastAction.taskLabel}" at {lastAction.at}</div>}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ fontSize: 12, padding: '4px 10px', background: G_LITE, color: G, borderRadius: 20, fontWeight: 500 }}>{userName}</div>
            <button onClick={() => setEditMode(!editMode)} style={{ fontSize: 11, padding: '4px 8px', background: editMode ? G : 'transparent', color: editMode ? '#fff' : '#9CA3AF', border: '1px solid #E5E7EB', borderRadius: 20, fontWeight: 500, cursor: 'pointer' }}>
              {editMode ? 'Done' : '✏️ Edit'}
            </button>
            <button onClick={logout} title="Switch user" style={{ fontSize: 11, padding: '4px 8px', background: 'transparent', color: '#9CA3AF', border: '1px solid #E5E7EB', borderRadius: 20, fontWeight: 500, cursor: 'pointer', lineHeight: 1.4 }}>↩ Exit</button>
          </div>
        </div>
      </div>
      ...
      <div className="main-content" style={{ maxWidth: 860, margin: '0 auto', padding: '1.5rem 1.25rem' }}>
          ...
          <div className="tab-buttons" style={{ display: 'flex', gap: 6 }}>
            {[['business', '💼', 'Business'], ['dev', '💻', 'Dev & Product']].map(([id, emoji, label]) => (
              <button key={id} onClick={() => { setTab(id); setPhase(0) }} style={{
                display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: tab === id ? 500 : 400,
                border: `${tab === id ? '1.5px' : '1px'} solid ${tab === id ? G : '#E5E7EB'}`,
                background: tab === id ? G_LITE : '#fff', color: tab === id ? G : '#6B7280'
          ...
          <div className="phase-buttons" style={{ display: 'flex', gap: 6 }}>
            {[[0, 'All'], [1, 'Phase 1'], [2, 'Phase 2']].map(([v, label]) => (
              <button key={v} onClick={() => setPhase(v)} style={{
                ...
                background: phaseFilter === v ? G : 'transparent', color: phaseFilter === v ? '#fff' : '#6B7280'
              }}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {editMode && (
            <button onClick={handleAddGroup} style={{ padding: '12px', border: '1px dashed #ccc', borderRadius: 12, background: 'transparent', color: '#666', cursor: 'pointer', fontWeight: 500 }}>
              + Add New Phase / Group
            </button>
          )}
        </div>
